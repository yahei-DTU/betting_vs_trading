...
def p_eta_max_fun(model_param):
    ...
    P_eta_max=P_stack(i_eta_max,T_op, Pr,AA)/model_param["max_electrolyzer_capacity"] # 0.28 at 90 degrees! But it changes with T!
...
def objective(Q, x_data, y_data, model_param):
    ...
    sse += 10000*(Q_0 - x_data[np.abs(x_data - p_eta_max_fun(model_param)*model_param['max_electrolyzer_capacity']).argmin()]**2 * Q_2)**2
...
def initialize_electrolyzer(model_param, config_dict):
    ...
    if config_dict["eff_type"]== 1: # HYP-MIL
        ...
        if quad_fit == 0: # Quadratic fit
            ...
            res = minimize(objective, x0 = [0,0,0], args=(p_min_max.flatten(), h_prod(i_min_max, T_op, Pr, AA).flatten(),model_param))
            D_2, D_1, D_0 = res.x
        ...
        # Left coefficiencys
        res = minimize(objective, x0 = [0,0,0], args=(p_vec1.flatten(), h_prod(i_vec1, T_op, Pr, AA).flatten(), model_param))
        D_2, D_1, D_0 = res.x
        model_param.update({'D_2': D_2}), model_param.update({'D_1': D_1}), model_param.update({'D_0': D_0})
        # Right coefficients
        res = minimize(objective_rhs, x0 = [0,0,0], args=(p_vec2.flatten(), h_prod(i_vec2, T_op, Pr, AA).flatten(), model_param['p_val'][1], model_param))
        E_2, E_1, E_0 = res.x
        model_param.update({'E_2': E_2}), model_param.update({'E_1': E_1}), model_param.update({'E_0': E_0})
    
    model_param.update({'a': a}), model_param.update({'b': b}), model_param.update({'Q_0': Q_0}), model_param.update({'Q_1': Q_1})
...
def plot_el_curves(model_param, config_dict):
    ...
    plt.legend()
    plt.grid()
    plt.xlim([model_param["p_min"]*model_param["max_electrolyzer_capacity"], model_param["max_electrolyzer_capacity"]])
    print(p_min_max[0])
    ...
    if config_dict["eff_type"]== 1:   #HYP-MIL
        ...
        for jj in range(0, N_s):
            ...
            plt.figure(fig_prod)
            plt.plot(x, (model_param["b"][jj]+model_param["a"][jj]*x), ":.", color='red', linewidth=2, label='Piecewise linearization', markersize=10)  
        ...
        for jj in range(0, N_s):
            ...
            plt.figure(fig_eff)
            plt.plot(p_vec, (model_param["a"][jj]+model_param["b"][jj]/(p_vec)), color='red', linewidth=2, label='Piecewise linearization', linestyle='--', dashes=(1, 1)) 
        plt.legend(['Nonlinear curve', 'Approximated curve'])
    ...
    if config_dict["eff_type"]==2: #HYP_L  
        ...
        # Approximated hydorgen production
        for jj in range(0, N_s):
            x=np.concatenate((np.array([model_param["p_min"]*model_param["max_electrolyzer_capacity"]]),np.array([model_param["max_electrolyzer_capacity"]])))
            # Plot hydrogen production in each segment
            plt.figure(fig_prod)
            plt.plot(x, (model_param["b"][jj]+model_param["a"][jj]*x), color='red', linewidth=1, label='Approximated curve', markersize=10)  
            plt.scatter(model_param['p_val'][jj], (model_param["b"][jj]+model_param["a"][jj]*model_param['p_val'][jj]), c= 'black')

        plt.scatter(model_param['p_val'][-1], (model_param["b"][-1]+model_param["a"][-1]*model_param['p_val'][-1]), c= 'black')
        plt.legend(['Nonlinear curve', 'Approximated curve'])
        # Approximated efficiency curve
        for jj in range(0, N_s):
            p_vec=np.linspace(model_param["p_val"][jj],model_param["p_val"][jj+1])
            plt.figure(fig_eff)
            plt.plot(p_vec, (model_param["a"][jj]+model_param["b"][jj]/(p_vec)), color='red', linewidth=1, label='Approximated curve', linestyle='--', dashes=(1, 1)) 
        plt.legend(['Nonlinear curve', 'Approximated curve'])
    ...
    if config_dict["eff_type"]== 3: #HYP-SOC
        ...
        p_val = np.concatenate((model_param["p_val"][:1], [model_param["p_val"][-1]])) # new p_val vector with only relevant points 
        ...
        for jj in range(0, N_s):
            ...
            plt.figure(fig_prod)
            plt.plot(p_vec, (model_param["D_0"]+model_param["D_1"]*p_vec+model_param["D_2"]*p_vec**2),":", color='blue', linewidth=2, label='Quadratic', markersize=10)    
        plt.legend()
        # Approximated efficiency curve
        for jj in range(0, N_s):
            ...
            p_vec=np.linspace(p_val[jj],p_val[jj+1])
            plt.figure(fig_eff)
            plt.plot(p_vec, (model_param["D_0"]/p_vec+model_param["D_1"]+model_param["D_2"]*p_vec),":", color='blue', linewidth=2, label='Quadratic', markersize=10)
        plt.legend()
        plt.show()
    ...
    if config_dict["eff_type"]== 4: # HYP-MISOC
        ...
        for jj in range(0, N_s):
            ...
            plt.figure(fig_prod)
            if jj == 0:
                plt.plot(p_vec, (model_param["D_0"]+model_param["D_1"]*p_vec+model_param["D_2"]*p_vec**2),":", color='red', linewidth=2, label='Left', markersize=10)  
            else:
                plt.plot(p_vec, (model_param["E_0"]+model_param["E_1"]*p_vec+model_param["E_2"]*p_vec**2),":", color='blue', linewidth=2, label='Right', markersize=10)      
        plt.legend()
        # Approximated efficiency curve
        for jj in range(0, N_s):
            p_vec=np.linspace(p_val[jj],p_val[jj+1])
            plt.figure(fig_eff)
            if jj == 0:
                plt.plot(p_vec, (model_param["D_0"]/p_vec+model_param["D_1"]+model_param["D_2"]*p_vec),":", color='red', linewidth=2, label='Left', markersize=10)
            else:
                plt.plot(p_vec, (model_param["E_0"]/p_vec+model_param["E_1"]+model_param["E_2"]*p_vec),":", color='blue', linewidth=2, label='Right', markersize=10)
        plt.legend()
        plt.show()

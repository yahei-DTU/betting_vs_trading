...
def test_policies(q_DA, q_H, data,x,model_param,price_quantile):
    ...
    for t in range(len(data)):
        ...
        for k in range(len(bid_lambda_DA)):
            ...
            if model_param["model_conditions"] == 1:
        ...
        if model_param["model_conditions"] == 2: 
            if p_IM[t] < 0:
                s1[t] = -p_IM[t]
                p_IM[t] = 0
        if model_param["model_conditions"] == 3:
